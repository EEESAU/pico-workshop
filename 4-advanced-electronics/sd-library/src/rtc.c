...
static void update_epochtime() {
    bool rc = aon_timer_get_time_calendar(&rtc_save.datetime);
    if (rc) {
        rtc_save.signature = 0xBABEBABE;
        struct tm timeinfo = {
            .tm_sec = rtc_save.datetime
                          .tm_sec, /* Seconds.	[0-60] (1 leap second) */
            .tm_min = rtc_save.datetime.tm_min,          /* Minutes.	[0-59] */
            .tm_hour = rtc_save.datetime.tm_hour,        /* Hours.	[0-23] */
            .tm_mday = rtc_save.datetime.tm_mday,         /* Day.		[1-31] */
            .tm_mon = rtc_save.datetime.tm_mon - 1,    /* Month.	[0-11] */
            .tm_year = rtc_save.datetime.tm_year - 1900, /* Year	- 1900.  */
        ...
        rtc_save.datetime.tm_wday = timeinfo.tm_wday;
...
void time_init() {
    struct tm t = {0, 0, 0, 0, 0, 0, 0};
    aon_timer_get_time_calendar(&t);
    if (!t.tm_year && rtc_save.datetime.tm_year) {
        ...
        if (rtc_save.signature == 0xBABEBABE &&
            ...
            // Set rtc
            aon_timer_set_time_calendar(&rtc_save.datetime);
        }
    }
}
...
DWORD get_fattime(void) {
    struct tm t = {0, 0, 0, 0, 0, 0, 0};
    bool rc = aon_timer_get_time_calendar(&t);
    if (!rc) return 0;
