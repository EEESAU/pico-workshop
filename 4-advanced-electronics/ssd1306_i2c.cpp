...
void SSD1306_send_buf(uint8_t buf[], int buflen) {
    ...
    auto temp_buf = std::vector<uint8_t>(buflen + 1);

    temp_buf[0] = 0x40;
    std::copy(buf, buf + buflen, temp_buf.begin() + 1);

    i2c_write_blocking(i2c_default, SSD1306_I2C_ADDR, temp_buf.data(), buflen + 1, false);
}
...
void SSD1306_scroll(bool on) {
    ...
    uint8_t cmds[] = {
        static_cast<uint8_t>(SSD1306_SET_HORIZ_SCROLL | 0x00),
        0x00, // dummy byte
        0x00, // start page 0
        0x00, // time interval
        ...
        0x00, // dummy byte
        0xFF, // dummy byte
        static_cast<uint8_t>(SSD1306_SET_SCROLL | (on ? 0x01 : 0)) // Start/stop scrolling
    };
...
static void WriteString(uint8_t *buf, int16_t x, int16_t y, const std::string& str) {
...
int main() {
    ...
    std::array<uint8_t, SSD1306_BUF_LEN> buf = {};
    render(buf.data(), &frame_area);
...
restart:
    ...
    for (int i = 0; i < 3; i++) {
        render(raspberry26x32.data(), &area);
    ...
    for (int i = 0; i < 2;i++) {
        for (int x = 0;x < SSD1306_WIDTH;x++) {
            DrawLine(buf.data(), x, 0,  SSD1306_WIDTH - 1 - x, SSD1306_HEIGHT - 1, pix);
            render(buf.data(), &frame_area);
        }

        for (int y = SSD1306_HEIGHT-1; y >= 0 ;y--) {
            DrawLine(buf.data(), 0, y, SSD1306_WIDTH - 1, SSD1306_HEIGHT - 1 - y, pix);
            render(buf.data(), &frame_area);
        }
        pix = false;
    }
